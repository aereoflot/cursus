...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/16 16:12:04 by ancrodri          #+#    #+#             */
/*   Updated: 2025/10/16 16:51:19 by ancrodri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		eof = 0;
		saved[fd] = ft_read_and_join(fd, saved[fd], &eof);
		if (!saved[fd])
			return (NULL);
		nl_pos = ft_find_newline(saved[fd]);
		if (nl_pos >= 0)
			return (ft_process_line(&saved[fd], nl_pos));
		if (!eof)
			continue ;
		return (ft_handle_eof(&saved[fd]));
	}
}
