...
class FunctionCallingEngine:
    def __init__(self) -> None:
        print("Initializing LLM model (Qwen/Qwen3-0.6B)... this may take a few seconds.")
    ...
    def process_prompt(self, prompt_text: str, available_functions: List[FunctionDef]) -> dict:
        # 1. Enriquecemos el prompt para que el modelo sepa qué funciones existen y elija bien
        system_context = "Select the correct function for the following request.\nAvailable functions:\n"
        ...
        while isinstance(raw_ids, list) and len(raw_ids) > 0 and isinstance(raw_ids[0], list):
            raw_ids = raw_ids[0]
        input_ids = [int(x) for x in raw_ids]

        generated_tokens = []

        # 2. Pre-calcular caminos obligatorios para la decodificación restringida
        allowed_sequences = []
        for func in available_functions:
            ...
            while isinstance(seq_ids, list) and len(seq_ids) > 0 and isinstance(seq_ids[0], list):
                seq_ids = seq_ids[0]
        ...
        # 3. Bucle de generación matemática
        ...
        # 4. Extracción estricta del JSON puro (aislamiento de corchetes balanceados)
        try:
        ...
        # Fallback en caso de que el modelo genere un JSON completamente roto
